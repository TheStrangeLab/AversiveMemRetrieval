...
% reported at Figure 1 and Supplementary Figure 1;
...
cd ~/data2github/
load 'Ga_Hippocampus_Timefreq.mat'
...
[Hn,Pn,CIn,STATSn] = ttest(mat(:,3),mat(:,4));
 ...
 de = (der-dekf)/stdemo
 dn= (dnr-dnkf)/stdneu
%% plot data Supplementary Fig. 1a
cfg = []; 
cfg.figure = 'gcf'
cfg.xlim = [-.5 1.5];
cfg.ylim = [35 150];
cfg.zlim = [-0.3 0.3];
h = figure;
subplot(3,2,1) ; ft_singleplotTFR(cfg,Ga_eHit); 
tit=title('eRHit');set(findobj(tit,'type','text'),'FontSize',36);
hold on
subplot(3,2,2); ft_singleplotTFR(cfg,Ga_nHit);
tit=title('nRHit');set(findobj(tit,'type','text'),'FontSize',36);
hold on
subplot(3,2,3); ft_singleplotTFR(cfg,Ga_eMiss);
tit=title('eMiss');set(findobj(tit,'type','text'),'FontSize',36);
hold on
subplot(3,2,4); ft_singleplotTFR(cfg,Ga_nMiss);
tit=title('nMiss');set(findobj(tit,'type','text'),'FontSize',36);
hold on
subplot(3,2,5); ft_singleplotTFR(cfg,Ga_eCrj);
tit=title('eCrj');set(findobj(tit,'type','text'),'FontSize',36);
hold on
subplot(3,2,6); ft_singleplotTFR(cfg,Ga_nCrj);
...
%% plot single subjects data as in Supplementary Fig 1b
...
%% plot stat for Supplementary Fig. 1c
...
%% plot single subjects data as in Supplementary Fig. 1c
clear mat
...
%% plot low freq data as in Supplementary Fig.2
cfg = []; 
cfg.figure = 'gcf'
cfg.xlim = [-.5 1.5];
cfg.ylim = [0 34];
